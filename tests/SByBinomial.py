...
def _generateArrayTree(tree: np.ndarray, t, i):
    if t == 0:
        return []
    ...
    SIZE = 2 ** t
    HALF = SIZE // 2
    newArray = np.zeros((SIZE, t), dtype=float)

    newArray[:HALF, 0] = tree[U]
    newArray[HALF:, 0] = tree[D]

    upArray = _generateArrayTree(tree, t - 1, U)
    downArray = _generateArrayTree(tree, t - 1, D)

    newArray[:HALF, 1:] = upArray
    newArray[HALF:, 1:] = downArray

    return newArray


def generateArrayTree(tree: np.ndarray):
    HALF = tree.shape[0] // 2
    newArray = _generateArrayTree(tree, HALF, HALF)

    return np.c_[np.full(newArray.shape[0], tree[HALF]), newArray]


def generatePrimeByBinomial(R, N, S0, r, T, volatily, K, isCall=False):
    am_option = BinomialCRRLattice(
        S0, K, r=r, T=T, N=N, sigma=volatily,
        is_put=(not isCall), is_am=True
    )
    
    am_option.setup_parameters()
    am_option.init_stock_price_tree()
    
    tree = generateArrayTree(am_option.STs)

    return am_option.price(), tree
